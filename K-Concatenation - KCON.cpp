...
void solve() {
    ll n, k, tot_sum = 0, max_ans = LONG_MIN, current_sum = 0, max_sum = LONG_MIN, pr_sum = LONG_MIN, su_sum = LONG_MIN, temp = 0;
    cin >> n >> k;
    vll arr(n);
    fori(0, n) {
        cin >> arr[i];
        tot_sum += arr[i];
        pr_sum = max(pr_sum, tot_sum);
    }
    max_ans = max(max_ans, tot_sum * k);
    fori(0, arr.size()) {
        current_sum += arr[i];
    ...
    for (int i = n - 1; i >= 0; i--) {
        temp += arr[i];
        su_sum = max(su_sum, temp);
    }
    if (tot_sum >= 0)
        max_ans = max(max_ans, tot_sum * (k - 2) + pr_sum + su_sum);
    else
        max_ans = max(max_ans, pr_sum + su_sum);
    cout << max_ans << "\n";
}
