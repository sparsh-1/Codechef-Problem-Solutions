// https://www.codechef.com/JULY221C/problems/FINDABC
/*
...
void solve() {
    ll n;
    cin >> n;
    vll arr(n + 1), posans(1, 0);
    fori(0, n + 1) {
        cin >> arr[i];
        if (arr[i] - arr[0] < 0)
            posans.pb(i);
    }
    sort(all(posans), greater<ll>());
    fori(0, posans.size()) {
        ll temp = arr[0] - posans[i];
        ll num2 = temp, num1 = 0;
        if (n < temp) {
            num2 = n;
            num1 = temp - n;
        ...
        while (num1 <= num2) {
            ...
            while (cnt <= arr.size() / 2)
                if (((num1 ^ cnt) + (num2 ^ cnt) + (posans[i] ^ cnt) == arr[cnt]) && ((num1 ^ (n - cnt)) + (num2 ^ (n - cnt)) + (posans[i] ^ (n - cnt)) == arr[(n - cnt)]))
                    cnt++;
                else
                    break;
            if (cnt == arr.size() / 2 + 1) {
                cout << posans[i] << " " << num1 << " " << num2 << "\n";
                return;
            }
            num1++, num2--;
        }
